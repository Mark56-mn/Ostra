 ...
 *
 * The gateway routes on the EXPLICIT selection it is handed. There is no
 * env-derived provider/model: each test states which provider/model it wants
 * via `use(...)` and the environment only supplies the credential.
 */
...
/** The explicit provider/model selection the next `generate()` call uses. */
let selection: { providerId: string; modelId: string } | null = null;

/** Declare the provider/model this test is exercising. */
function use(providerId: string, modelId: string): void {
  selection = { providerId, modelId };
}

/** callProvider with the current explicit selection applied. */
function generate(options: Record<string, unknown> = {}) {
  return callProvider(MESSAGES, {
    ...(selection ? { providerOverride: selection } : {}),
    ...options,
  });
}
...
describe("openai-compatible gateway", async () => {
  ...
  it("constructs a correct OpenAI-compatible request", async () => {
    use("openrouter", "nvidia/nemotron-3.5-lightning:free");
  ...
  it("treats 401 as a non-retryable auth failure without echoing the key", async () => {
    use("groq", "openai/gpt-oss-120b");
    setEnv({ GROQ_API_KEY: "secret-key-value" });
    resetProviderConfig();
  ...
  it("rejects when the provider key is missing", async () => {
    use("mistral", "mistral-small-latest");
    setEnv({});
    resetProviderConfig();
  ...
  it("times out and reports model_timeout", async () => {
    ...
    use("openrouter", "nvidia/nemotron-3.5-lightning:free");
    setEnv({ OPENROUTER_API_KEY: "k", MODEL_TIMEOUT_MS: "5000" });
    resetProviderConfig();
  ...
  it("refuses to guess a provider when no selection is supplied", async () => {
    setEnv({ OPENROUTER_API_KEY: "k" });
    resetProviderConfig();
    ...
    await assert.rejects(
      ...
      (error: Error & { code?: string; message?: string }) => {
        ...
        assert.match(String(error.message), /No AI model selected/);
        return true;
      },
    );
  });
  ...
  it("supports the per-request provider override (Stage 2)", async () => {
    setEnv({ OPENROUTER_API_KEY: "or-key", GROQ_API_KEY: "groq-key" });
    resetProviderConfig();
    ...
    // (the per-request selection wins over anything else)
...
describe("gemini adapter", async () => {
  ...
  it("builds a native Gemini request with systemInstruction and role mapping", async () => {
    use("gemini", "gemini-flash-latest");
  ...
  it("merges consecutive same-role messages for Gemini", async () => {
    ...
    await callProvider(
      ...
      { providerOverride: { providerId: "gemini", modelId: "gemini-flash-latest" } },
    );
